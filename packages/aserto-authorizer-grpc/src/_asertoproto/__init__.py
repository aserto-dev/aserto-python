...
class FieldMetadata:
    ...
    def get(field: dataclasses.Field) -> "FieldMetadata":
        ...
        return field.metadata["_asertoproto"]
...
def dataclass_field(
    ...
    *,
    ...
    return dataclasses.field(
        default=PLACEHOLDER,
        metadata={
            "_asertoproto": FieldMetadata(number, proto_type, map_types, group, wraps)
        },
    )
...
class Message(ABC):
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, meta in self.__asertoproto.meta_by_field_name.items():
    ...
    def __eq__(self, other) -> bool:
        ...
        for field_name in self.__asertoproto.meta_by_field_name:
    ...
    def __repr__(self) -> str:
        parts = [
            ...
            for field_name in self.__asertoproto.sorted_field_names
    ...
    def __setattr__(self, attr: str, value: Any) -> None:
        ...
        if hasattr(self, "_group_current"):  # __post_init__ had already run
            if attr in self.__asertoproto.oneof_group_by_field:
                group = self.__asertoproto.oneof_group_by_field[attr]
                for field in self.__asertoproto.oneof_field_by_group[group]:
                    if field.name == attr:
    ...
    @property
    def __asertoproto(self) -> ProtoClassMetadata:
        """
        Lazy initialize metadata for each protobuf class.
        It may be initialized multiple times in a multi-threaded environment,
        but that won't affect the correctness.
        """
        meta = getattr(self.__class__, "__asertoproto_meta", None)
        if not meta:
            meta = ProtoClassMetadata(self.__class__)
            self.__class__.__asertoproto_meta = meta  # type: ignore
        return meta
    ...
    def _get_field_default(self, field_name: str) -> Any:
        return self.__asertoproto.default_gen[field_name]()
    ...
    def _postprocess_single(
        ...
        if wire_type == WIRE_VARINT:
            ...
            if meta.proto_type == TYPE_STRING:
                ...
                cls = self.__asertoproto.cls_by_field[field_name]
                ...
                value = self.__asertoproto.cls_by_field[field_name]().parse(value)
    ...
    def to_dict(
        ...
        defaults = self.__asertoproto.default_gen
        for field_name, meta in self.__asertoproto.meta_by_field_name.items():
    ...
    def from_dict(self: T, value: Dict[str, Any]) -> T:
        ...
        for key in value:
            ...
            meta = self.__asertoproto.meta_by_field_name.get(field_name)
            if not meta:
                continue
            ...
            if value[key] is not None:
                if meta.proto_type == TYPE_MESSAGE:
                    ...
                    v = getattr(self, field_name)
                    cls = self.__asertoproto.cls_by_field[f"{field_name}.value"]
                    for k in value[key]:
                    ...
                    if meta.proto_type in INT_64_TYPES:
                        ...
                        enum_cls = self.__asertoproto.cls_by_field[field_name]
                        if isinstance(v, list):
