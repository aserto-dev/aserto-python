...
![](https://github.com/danielgtaylor/python-_asertoproto/workflows/CI/badge.svg)
> :octocat: If you're reading this on github, please be aware that it might mention unreleased features! See the latest released README on [pypi](https://pypi.org/project/_asertoproto/).
...
pip install "_asertoproto[compiler]"

# Install just the library (to use the generated code output)
pip install _asertoproto
```

*Betterproto* is under active development. To install the latest beta version, use `pip install --pre _asertoproto`.
...
```sh
mkdir lib
protoc -I . --python__asertoproto_out=lib example.proto
```
...
python -m grpc_tools.protoc -I . --python__asertoproto_out=lib example.proto
```
...
@dataclass
class Greeting(_asertoproto.Message):
    """Greeting represents a message you can tell a user."""

    message: str = _asertoproto.string_field(1)
```
...
```
python -m grpc_tools.protoc -I . --python__asertoproto_out=. echo.proto
```
...
```python
MyMessage().to_dict(casing=_asertoproto.Casing.SNAKE)
```
...
Use `_asertoproto.serialized_on_wire(message)` to determine if it was sent. This is a little bit different from the official Google generated Python code, and it lives outside the generated `Message` class to prevent name clashes. Note that it **only** supports Proto 3 and thus can **only** be used to check if `Message` fields are set. You cannot check if a scalar was sent on the wire.
...
>>> _asertoproto.serialized_on_wire(mymessage.myfield)
```
...
You can use `_asertoproto.which_one_of(message, group_name)` to determine which of the fields was set. It returns a tuple of the field name and value, or a blank string and `None` if unset.

```py
>>> test = Test()
>>> _asertoproto.which_one_of(test, "foo")
["", None]
...
# New way (this project)
>>> _asertoproto.which_one_of(message, "group")
["foo", "foo's value"]
```
...
| `google.protobuf.*`         | `_asertoproto.lib.google.protobuf.*`      | `None`                 |
...
- _Join us on [Slack](https://join.slack.com/t/_asertoproto/shared_invite/zt-f0n0uolx-iN8gBNrkPxtKHTLpG3o1OQ)!_
...
- Create a new directory `_asertoproto/tests/inputs/<name>`
  - add `<name>.proto`  with a message called `Test`
  - add `<name>.json` with some test data (optional)

It will be picked up automatically when you run the tests.

- See also: [Standard Tests Development Guide](_asertoproto/tests/README.md)
...
Betterproto includes compiled versions for Google's well-known types at [_asertoproto/lib/google](_asertoproto/lib/google).
...
```sh
protoc \
    --plugin=protoc-gen-custom=src/_asertoproto/plugin/main.py \
    --custom_opt=INCLUDE_GOOGLE \
    --custom_out=src/_asertoproto/lib \
    -I /usr/local/include/ \
...
Join us on [Slack](https://join.slack.com/t/_asertoproto/shared_invite/zt-f0n0uolx-iN8gBNrkPxtKHTLpG3o1OQ)!
